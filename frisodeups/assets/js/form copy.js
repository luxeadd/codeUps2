...
$(function () {
  ...
  //必須項目チェック用　定数の作成
  var target = '[aria-required="true"]';
  //承認が不要の場合は削除、ネーム名もひとそれぞれ違う可能性あり
  var target2 = '[name="acceptance-86"]';
  var target3 = "[aria-describedby]";

  //キー操作、状態変化をトリガーに実行
  $(target + "," + target2 + "," + target3).on("keydown keyup keypress change", function () {
    //判定用のフラグ
    var flag = true;
    var flag2 = $(target2).is(":checked");
    var flag3 = $(target3).attr("aria-describedby");
